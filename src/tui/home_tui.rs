...
fn draw_home_ui<B: Backend>(f: &mut Frame, current_frame_index: usize, theme: &Theme) {
    let size = f.area();
...
pub async fn run_home_page<B: Backend>(
    ...
    let mut animation_frame_index = 0;
    ...
    loop {
        ...
        terminal.draw(|f| {
            draw_home_ui::<B>(f, animation_frame_index, &current_theme);
        })?;
